...
Properties {
	...
	[space]
	[space]
	_CloudBottom("Cloud Bottom", Range(0,8000)) = 0
	_CloudTop("Cloud Top", Range(0,8000)) = 1
	[NoScaleOffset] _Noise("Noise", 3D) = "" {}
	[NoScaleOffset] _DetailNoise("Detail Noise", 3D) = "" {}
	[Space]
	[Space]
	_Turbulence("Turbulence", Range(0,1)) = 0.5
	_ScatterCoeff("Scatter Coeff", float) = 20
	_NoiseA ("Noise Weight R", Range(-1,1)) = 1
	_NoiseB ("Noise Weight G", Range(-1,1)) = 1
	_NoiseC ("Noise Weight B", Range(-1,1)) = 1
	_NoiseD ("Noise Weight A", Range(-1,1)) = 1
	[Space]
	[Space]
	_NoiseFreqA ("Noise Freq R", Range(0,0.7)) = 1
	_NoiseFreqB ("Noise Freq G", Range(0,0.7)) = 1
	_NoiseFreqC ("Noise Freq B", Range(0,0.7)) = 1
	_NoiseFreqD ("Noise Freq A", Range(0,0.7)) = 1
	[Space]
	[Space]
...
SubShader {
	...
	Pass {
		...
		sampler3D _Noise, _DetailNoise;
		half _CloudBottom, _CloudTop;
		...
		half _NoiseA, _NoiseB, _NoiseC, _NoiseD, _NoiseBias, _HeightBias;
		half _NoiseFreqA, _NoiseFreqB, _NoiseFreqC, _NoiseFreqD;
		half _Turbulence, _ScatterCoeff;
...
//				for(int i=0; i<int(kSamples); i++)
		...
		inline half random(float2 uv)
		{
			return frac(43758.5453 * sin(dot(float2(12.9898, 78.233), uv)));
		}

		half sampleNoise(half3 pos) {
			half scale = 0.0004;
			half bias = _NoiseBias;
			half4 N;
			N.b = tex3D(_Noise, scale*pos*_NoiseFreqC) * _Turbulence;
			N.r = tex3D(_Noise, N.b+scale*pos*_NoiseFreqA);
			N.g = tex3D(_Noise, N.b+scale*pos*_NoiseFreqB);
			N.a = tex3D(_Noise, N.b+scale*pos*_NoiseFreqD);
			half4 D = tex3D(_DetailNoise, scale*(pos+(_Time.x*1000))) * _Turbulence;
			N += D;
			half n = saturate((N.r * _NoiseA + N.g * _NoiseB + N.b * _NoiseC + N.a * _NoiseD) + bias);
			n *= smoothstep(0, _CloudBottom, pos.y);
			n *= 1-smoothstep(0, _CloudTop, pos.y);
			return n;
		}

		float beerPowder(float depth)
		{
			const float _Beer = 0.8;
			return exp(-_Beer * depth) * (1 - exp(-_Beer * 2 * depth));
		}

		inline float henyeyGreenstein(float cosine)
		{
			float g = 0.5;
			float g2 = g * g;
			return 0.5 * (1 - g2) / pow(1 + g2 - 2 * g * cosine, 1.5);
		}

		half3 calcClouds(half3 ray, half3 col, half3 sunCol) {
			half3 cameraPos = float3(0,kInnerRadius + kCameraHeight,0);
			half3 pos = cameraPos;
			pos.xz += _Time[0]*100;
			half3 stride = (ray * 4700) / 32.0;
			half density = 0;
			half stepV = 1.0 / 32;
			half3 accum = 0;
			half3 sunRay = -_WorldSpaceLightPos0.xyz * stepV;
			float hg = henyeyGreenstein(dot(ray, _WorldSpaceLightPos0.xyz));
			[loop] for(int i=0; i<32; i++) {
				half n = sampleNoise(pos);
				half a = 0;
				for(int j=0; j<4; j++) {
					a += sampleNoise(pos + sunRay + random(pos.xz)) * (_CloudTop-_CloudBottom);
				}
				a = beerPowder(a * 0.25);
				pos += stride;
				density += n * stepV;
				accum += stepV * sunCol * density * a;
				float scatter = density * _ScatterCoeff * hg;
                accum += sunCol * scatter * beerPowder(density);
				
			}
			
			
			return lerp(col,accum,density);
		...
		half3 ray = normalize(mul((float3x3)unity_ObjectToWorld, IN.vertex));
			...
			#endif
				col += mie * IN.sunColor;
				col = calcClouds(-ray, col, IN.sunColor);
			}
		#endif
